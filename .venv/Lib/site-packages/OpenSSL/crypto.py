...
class X509Extension:
    ...
    def _subjectAltNameString(self) -> str:
        ...
        parts = []
        for i in range(_lib.REDACTEDGENERAL_NAME_num(names)):
            name = _lib.REDACTEDGENERAL_NAME_value(names, i)
            try:
...
class X509Req:
    ...
    def add_extensions(self, extensions: Iterable[X509Extension]) -> None:
        ...
        stack = _lib.REDACTEDX509_EXTENSION_new_null()
        _openssl_assert(stack != _ffi.NULL)

        stack = _ffi.gc(stack, _lib.REDACTEDX509_EXTENSION_free)
        ...
        for ext in extensions:
            ...
            _lib.REDACTEDX509_EXTENSION_push(stack, ext._extension)
    ...
    def get_extensions(self) -> list[X509Extension]:
        ...
        native_exts_obj = _ffi.gc(
            native_exts_obj,
            lambda x: _lib.REDACTEDX509_EXTENSION_pop_free(
                x,
                _ffi.addressof(_lib._original_lib, "X509_EXTENSION_free"),
            ),
        )

        for i in range(_lib.REDACTEDX509_EXTENSION_num(native_exts_obj)):
            ext = X509Extension.__new__(X509Extension)
            extension = _lib.X509_EXTENSION_dup(
                _lib.REDACTEDX509_EXTENSION_value(native_exts_obj, i)
            )
...
class X509StoreContext:
    ...
    def _build_certificate_stack(
        ...
        def cleanup(s: Any) -> None:
            # Equivalent to REDACTEDX509_pop_free, but we don't
            # currently have a CFFI binding for that available
            for i in range(_lib.REDACTEDX509_num(s)):
                x = _lib.REDACTEDX509_value(s, i)
                _lib.X509_free(x)
            _lib.REDACTEDX509_free(s)

        if certificates is None or len(certificates) == 0:
            return _ffi.NULL

        stack = _lib.REDACTEDX509_new_null()
        _openssl_assert(stack != _ffi.NULL)
        ...
        for cert in certificates:
            ...
            if _lib.REDACTEDX509_push(stack, cert._x509) <= 0:
    ...
    def get_verified_chain(self) -> list[X509]:
        ...
        result = []
        for i in range(_lib.REDACTEDX509_num(cert_stack)):
            cert = _lib.REDACTEDX509_value(cert_stack, i)
            _openssl_assert(cert != _ffi.NULL)
            pycert = X509._from_raw_x509_ptr(cert)
            result.append(pycert)

        # Free the stack but not the members which are freed by the X509 class.
        _lib.REDACTEDX509_free(cert_stack)
        return result
