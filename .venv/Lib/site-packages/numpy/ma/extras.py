...
__all__ = [
    ...
    'isin', 'in1d', 'intersect1d', 'maREDACTEDcols', 'maREDACTEDrowcols', 'maREDACTEDrows',
...
from .core import (
    ...
    getmask, getmaskarray, make_maREDACTEDdescr, masked, masked_array, maREDACTEDor,
...
def masked_all(shape, dtype=float):
    ...
    a = masked_array(np.empty(shape, dtype),
                     mask=np.ones(shape, make_maREDACTEDdescr(dtype)))
    return a
...
def masked_all_like(arr):
    ...
    a._mask = np.ones(a.shape, dtype=make_maREDACTEDdescr(a.dtype))
    return a
...
def maREDACTEDrowcols(a, axis=None):
    """
    ...
    See Also
    --------
    maREDACTEDrows : Mask rows of a 2D array that contain masked values.
    maREDACTEDcols : Mask cols of a 2D array that contain masked values.
    ...
    if a.ndim != 2:
        raise NotImplementedError("maREDACTEDrowcols works for 2D arrays only.")
    m = getmask(a)
...
def maREDACTEDrows(a, axis=np._NoValue):
    """
    Mask rows of a 2D array that contain masked values.

    This function is a shortcut to ``maREDACTEDrowcols`` with `axis` equal to 0.

    See Also
    --------
    maREDACTEDrowcols : Mask rows and/or columns of a 2D array.
    masked_where : Mask where a condition is met.
    ...
    return maREDACTEDrowcols(a, 0)


def maREDACTEDcols(a, axis=np._NoValue):
    """
    Mask columns of a 2D array that contain masked values.

    This function is a shortcut to ``maREDACTEDrowcols`` with `axis` equal to 1.
...
def polyfit(x, y, deg, rcond=None, full=False, w=None, cov=False):
    ...
    m = getmask(x)
    if y.ndim == 1:
        m = maREDACTEDor(m, getmask(y))
    elif y.ndim == 2:
        my = getmask(maREDACTEDrows(y))
        if my is not nomask:
            m = maREDACTEDor(m, my[:, 0])
    else:
