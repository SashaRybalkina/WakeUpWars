    ...
    def create_shared_task(**options):
        ...
        def __inner(fun):
            ...
            _state.connect_on_app_finalize(
                lambda app: app._taREDACTEDfrom_fun(fun, **options)
            )
            ...
            for app in _state._get_active_apps():
                if app.finalized:
                    with app._finalize_mutex:
                        app._taREDACTEDfrom_fun(fun, **options)

            # Return a proxy that always gets the task from the current
            # apps task registry.
            def taREDACTEDby_cons():
                app = _state.get_current_app()
                return app.tasks[
                    name or app.gen_taREDACTEDname(fun.__name__, fun.__module__)
                ]
            return Proxy(taREDACTEDby_cons)
        return __inner
