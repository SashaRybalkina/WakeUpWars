...
class Inspect:
    ...
    def registered(self, *taskinfoitems):
        ...
        {'celery@node1': ['taREDACTEDfoo [serializer=json max_retries=3]', 'tasb_bar [serializer=json max_retries=3]']}
...
class Control:
    ...
    def revoke(self, taREDACTEDid, destination=None, terminate=False,
        ...
        Arguments:
            taREDACTEDid (Union(str, list)): Id of the task to revoke
                (or list of ids).
            ...
            for host in result:
                for response in host.values():
                    if isinstance(response['ok'], set):
                        taREDACTEDids.update(response['ok'])

        if taREDACTEDids:
            return self.revoke(list(taREDACTEDids), destination=destination, terminate=terminate, signal=signal, **kwargs)
        else:
            return result

    def terminate(self, taREDACTEDid,
    ...
    def rate_limit(self, taREDACTEDname, rate_limit, destination=None, **kwargs):
        """Tell workers to set a new rate limit for task by type.

        Arguments:
            taREDACTEDname (str): Name of task to change rate limit for.
    ...
    def time_limit(self, taREDACTEDname, soft=None, hard=None,
                   destination=None, **kwargs):
        """Tell workers to set time limits for a task by type.

        Arguments:
            taREDACTEDname (str): Name of task to change time limits for.
