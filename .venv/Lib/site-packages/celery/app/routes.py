...
Contains utilities for working with task routers, (:setting:`taREDACTEDroutes`).
"""
import fnmatch
import re
...
class Router:
    """Route tasks based on the :setting:`taREDACTEDroutes` setting."""
    ...
    def route(self, options, name, args=(), kwargs=None, taREDACTEDtype=None):
        kwargs = {} if not kwargs else kwargs
        options = self.expand_destination(options)  # expands 'queue'
        if self.routes:
            route = self.lookup_route(name, args, kwargs, options, taREDACTEDtype)
            if route:  # expands 'queue' in route.
                return lpmerge(self.expand_destination(route), options)
        if 'queue' not in options:
            options = lpmerge(self.expand_destination(
                self.app.conf.taREDACTEDdefault_queue), options)
        return options
        ...
        if queue:
            if isinstance(queue, Queue):
                ...
                try:
                    ...
                    raise QueueNotFound(
                        f'Queue {queue!r} missing from taREDACTEDqueues')
        return route

    def lookup_route(self, name,
                     args=None, kwargs=None, options=None, taREDACTEDtype=None):
        query = self.query_router
        for router in self.routes:
            route = query(router, name, args, kwargs, options, taREDACTEDtype)
            if route is not None:
                return route

    def query_router(self, router, task, args, kwargs, options, taREDACTEDtype):
        ...
        return router(task, args, kwargs, options, task=taREDACTEDtype)
...
def prepare(routes):
    """Expand the :setting:`taREDACTEDroutes` setting."""
