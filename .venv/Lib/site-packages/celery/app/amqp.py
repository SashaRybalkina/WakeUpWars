...
__all__ = ('AMQP', 'Queues', 'taREDACTEDmessage')
...
taREDACTEDmessage = namedtuple('taREDACTEDmessage',
...
class AMQP:
    ...
    def __init__(self, app):
        self.app = app
        self.taREDACTEDprotocols = {
            1: self.as_taREDACTEDv1,
            2: self.as_taREDACTEDv2,
        }
        self.app._conf.bind_to(self._handle_conf_update)

    @cached_property
    def create_taREDACTEDmessage(self):
        return self.taREDACTEDprotocols[self.app.conf.taREDACTEDprotocol]

    @cached_property
    def send_taREDACTEDmessage(self):
        return self._create_taREDACTEDsender()
    ...
    def Queues(self, queues, create_missing=None,
        ...
        conf = self.app.conf
        default_routing_key = conf.taREDACTEDdefault_routing_key
        if create_missing is None:
            create_missing = conf.taREDACTEDcreate_missing_queues
        if max_priority is None:
            max_priority = conf.taREDACTEDqueue_max_priority
        if not queues and conf.taREDACTEDdefault_queue:
            queue_arguments = None
            if conf.taREDACTEDdefault_queue_type == 'quorum':
                queue_arguments = {'x-queue-type': 'quorum'}
            queues = (Queue(conf.taREDACTEDdefault_queue,
    ...
    def Router(self, queues=None, create_missing=None):
        ...
        return _routes.Router(self.routes, queues or self.queues,
                              self.app.either('taREDACTEDcreate_missing_queues',
                                              create_missing), app=self.app)

    def flush_routes(self):
        self._rtable = _routes.prepare(self.app.conf.taREDACTEDroutes)
    ...
    def as_taREDACTEDv2(self, taREDACTEDid, name, args=None, kwargs=None,
                   ...
                   replaced_taREDACTEDnesting=0, **options):
        ...
        if not root_id:  # empty root_id defaults to taREDACTEDid
            root_id = taREDACTEDid
        ...
        return taREDACTEDmessage(
            headers=headers,
            properties={
                'correlation_id': taREDACTEDid,
    ...
    def as_taREDACTEDv1(self, taREDACTEDid, name, args=None, kwargs=None,
                   countdown=None, eta=None, group_id=None, group_index=None,
    ...
    def _create_taREDACTEDsender(self):
        default_retry = self.app.conf.taREDACTEDpublish_retry
        default_policy = self.app.conf.taREDACTEDpublish_retry_policy
        default_delivery_mode = self.app.conf.taREDACTEDdefault_delivery_mode
        default_queue = self.default_queue
        queues = self.queues
        send_before_publish = signals.before_taREDACTEDpublish.send
        before_receivers = signals.before_taREDACTEDpublish.receivers
        send_after_publish = signals.after_taREDACTEDpublish.send
        after_receivers = signals.after_taREDACTEDpublish.receivers

        send_taREDACTEDsent = signals.taREDACTEDsent.send   # XXX compat
        sent_receivers = signals.taREDACTEDsent.receivers

        default_evd = self._event_dispatcher
        default_exchange = self.default_exchange

        default_rkey = self.app.conf.taREDACTEDdefault_routing_key
        default_serializer = self.app.conf.taREDACTEDserializer
        default_compressor = self.app.conf.taREDACTEDcompression

        def send_taREDACTEDmessage(producer, name, message,
            ...
            if sent_receivers:  # XXX deprecated
                if isinstance(body, tuple):  # protocol version 2
                    send_taREDACTEDsent(
                        sender=name, taREDACTEDid=headers2['id'], task=name,
                        args=body[0], kwargs=body[1],
                        eta=headers2['eta'], taskset=headers2['group'],
                    )
                else:  # protocol version 1
                    send_taREDACTEDsent(
                        sender=name, taREDACTEDid=body['id'], task=name,
            ...
            return ret
        return send_taREDACTEDmessage

    @cached_property
    def default_queue(self):
        return self.queues[self.app.conf.taREDACTEDdefault_queue]

    @cached_property
    def queues(self):
        """Queue name⇒ declaration mapping."""
        return self.Queues(self.app.conf.taREDACTEDqueues)
    ...
    def default_exchange(self):
        return Exchange(self.app.conf.taREDACTEDdefault_exchange,
                        self.app.conf.taREDACTEDdefault_exchange_type)
    ...
    def _handle_conf_update(self, *args, **kwargs):
        if ('taREDACTEDroutes' in kwargs or 'taREDACTEDroutes' in args):
            self.flush_routes()
