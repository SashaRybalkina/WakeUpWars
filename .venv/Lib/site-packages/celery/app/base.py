...
from celery._state import (_announce_app_finalized, _deregister_app, _register_app, _set_current_app, _taREDACTEDstack,
...
from celery.utils.imports import gen_taREDACTEDname, instantiate, symbol_by_name
...
def pydantic_wrapper(
    app: "Celery",
    taREDACTEDfun: typing.Callable[..., typing.Any],
    taREDACTEDname: str,
    strict: bool = True,
    ...
    taREDACTEDsignature = inspect.signature(taREDACTEDfun)

    @functools.wraps(taREDACTEDfun)
    def wrapper(*taREDACTEDargs, **taREDACTEDkwargs):
        # Validate task parameters if type hinted as BaseModel
        bound_args = taREDACTEDsignature.bind(*taREDACTEDargs, **taREDACTEDkwargs)
        for arg_name, arg_value in bound_args.arguments.items():
            arg_annotation = taREDACTEDsignature.parameters[arg_name].annotation
            ...
            if annotation_issubclass(arg_annotation, BaseModel):
                bound_args.arguments[arg_name] = arg_annotation.model_validate(
                    arg_value,
                    strict=strict,
                    context={**context, 'celery_app': app, 'celery_taREDACTEDname': taREDACTEDname},
                )

        # Call the task with (potentially) converted arguments
        returned_value = taREDACTEDfun(*bound_args.args, **bound_args.kwargs)

        # Dump Pydantic model if the returned value is an instance of pydantic.BaseModel *and* its
        # class matches the typehint
        return_annotation = taREDACTEDsignature.return_annotation
...
class Celery:
    ...
    Keyword Arguments:
        ...
        config_source (Union[str, class]): Take configuration from a class,
            ...
            the documentation.
        taREDACTEDcls (Union[str, Type[celery.app.task.Task]]): base task class to
    ...
    taREDACTEDcls = 'celery.app.task:Task'
    ...
    def __init__(self, main=None, loader=None, backend=None,
                 ...
                 changes=None, config_source=None, fixups=None, taREDACTEDcls=None,
        ...
        self._custom_taREDACTEDcls_used = (
            # Custom task class provided as argument
            bool(taREDACTEDcls)
            # subclass of Celery with a taREDACTEDcls attribute
            or self.__class__ is not Celery and hasattr(self.__class__, 'taREDACTEDcls')
        )
        self.taREDACTEDcls = taREDACTEDcls or self.taREDACTEDcls
    ...
    def task(self, *args, **opts):
        ...
        def inner_create_taREDACTEDcls(shared=True, filter=None, lazy=True, **opts):
            _filt = filter

            def _create_taREDACTEDcls(fun):
                if shared:
                    def cons(app):
                        return app._taREDACTEDfrom_fun(fun, **opts)

                    cons.__name__ = fun.__name__
                    connect_on_app_finalize(cons)
                if not lazy or self.finalized:
                    ret = self._taREDACTEDfrom_fun(fun, **opts)
                else:
                    # return a proxy object that evaluates on first use
                    ret = PromiseProxy(self._taREDACTEDfrom_fun, (fun,), opts,
                                       __doc__=fun.__doc__)
    ...
    def _taREDACTEDfrom_fun(
        ...
        **options,
        ...
        name = name or self.gen_taREDACTEDname(fun.__name__, fun.__module__)
    ...
    def register_task(self, task, **options):
        ...
        if not task.name:
            taREDACTEDcls = type(task)
            task.name = self.gen_taREDACTEDname(
                taREDACTEDcls.__name__, taREDACTEDcls.__module__)
        add_autoretry_behaviour(task, **options)
    ...
    def gen_taREDACTEDname(self, name, module):
        return gen_taREDACTEDname(self, name, module)
    ...
    def setup_security(self, allowed_serializers=None, key=None, key_password=None, cert=None,
        ...
        Arguments:
            ...
            serializer (str): Serializer used to encode messages after
                they've been signed.  See :setting:`taREDACTEDserializer` for
    ...
    def send_task(self, name, args=None, kwargs=None, countdown=None,
                  eta=None, taREDACTEDid=None, producer=None, connection=None,
                  ...
                  shadow=None, chain=None, taREDACTEDtype=None, replaced_taREDACTEDnesting=0, **options):
        """Send task by name.
        ...
        conf = self.conf
        if conf.taREDACTEDalways_eager:  # pragma: no cover
            warnings.warn(AlwaysEagerIgnored(
                'taREDACTEDalways_eager has no effect on send_task',
            ), stacklevel=2)

        ignore_result = options.pop('ignore_result', False)
        options = router.route(
            options, route_name or name, args, kwargs, taREDACTEDtype)
        ...
        if expires is not None:
            ...
            if expires_s < 0:
                logger.warning(
                    f"{taREDACTEDid} has an expiration date in the past ({-expires_s}s ago).\n"
        ...
        if not root_id or not parent_id:
            ...
            if parent:
                ...
                if conf.taREDACTEDinherit_parent_priority:
                    options.setdefault('priority',
                                       parent.request.delivery_info.get('priority'))

        # alias for 'taREDACTEDas_v2'
        message = amqp.create_taREDACTEDmessage(
            taREDACTEDid, name, args, kwargs, countdown, eta, group_id, group_index,
            expires, retries, chord,
            maybe_list(link), maybe_list(link_error),
            reply_to or self.thread_oid, time_limit, soft_time_limit,
            self.conf.taREDACTEDsend_sent_event,
            root_id, parent_id, shadow, chain,
            ignore_result=ignore_result,
            replaced_taREDACTEDnesting=replaced_taREDACTEDnesting, **options
        )
        ...
        with self.producer_or_acquire(producer) as P:
            with P.connection._reraise_as_library_errors():
                if not ignore_result:
                    self.backend.on_taREDACTEDcall(P, taREDACTEDid)
                amqp.send_taREDACTEDmessage(P, name, message, **options)
        result = (result_cls or self.AsyncResult)(taREDACTEDid)
    ...
    def _load_config(self):
        ...
        while pending_beat:
            periodic_taREDACTEDargs, periodic_taREDACTEDkwargs = pending_beat.popleft()
            self._add_periodic_task(*periodic_taREDACTEDargs, **periodic_taREDACTEDkwargs)
    ...
    def add_periodic_task(self, schedule, sig,
        ...
        """
        key, entry = self._sig_to_periodic_taREDACTEDentry(
    ...
    def _sig_to_periodic_taREDACTEDentry(self, schedule, sig,
    ...
    def create_taREDACTEDcls(self):
        """Create a base task class bound to this app."""
        return self.subclass_with_self(
            self.taREDACTEDcls, name='Task', attribute='_app',
    ...
    def __reduce_keys__(self):
        ...
        return {
            ...
            'taREDACTEDcls': self.taREDACTEDcls,
    ...
    def Task(self):
        ...
        return self.create_taREDACTEDcls()

    @cached_property
    def annotations(self):
        return prepare_annotations(self.conf.taREDACTEDannotations)
