...
send_prerun = signals.taREDACTEDprerun.send
send_postrun = signals.taREDACTEDpostrun.send
send_success = signals.taREDACTEDsuccess.send
...
def taREDACTEDhas_custom(task, attr):
...
def get_taREDACTEDname(request, default):
...
class TraceInfo:
    ...
    def handle_retry(self, task, req, store_errors=True, **kwargs):
        ...
        try:
            ...
            signals.taREDACTEDretry.send(sender=task, request=req,
                                    reason=reason, einfo=einfo)
            info(LOG_RETRY, {
                'id': req.id,
                'name': get_taREDACTEDname(req, task.name),
                'exc': str(reason),
            })
            return einfo
        finally:
    ...
    def handle_failure(self, task, req, store_errors=True, call_errbacks=True):
        ...
        signals.taREDACTEDfailure.send(sender=task, taREDACTEDid=req.id,
...
def build_tracer(name, task, loader=None, hostname=None, store_errors=True,
    ...
    fun = task if taREDACTEDhas_custom(task, '__call__') else task.run
    ...
    deduplicate_successful_tasks = ((app.conf.taREDACTEDacks_late or task.acks_late)
                                    and app.conf.worker_deduplicate_successful_tasks
                                    and app.backend.persistent)

    hostname = hostname or gethostname()
    inherit_parent_priority = app.conf.taREDACTEDinherit_parent_priority

    loader_taREDACTEDinit = loader.on_taREDACTEDinit
    loader_cleanup = loader.on_process_cleanup

    taREDACTEDbefore_start = None
    taREDACTEDon_success = None
    taREDACTEDafter_return = None
    if taREDACTEDhas_custom(task, 'before_start'):
        taREDACTEDbefore_start = task.before_start
    if taREDACTEDhas_custom(task, 'on_success'):
        taREDACTEDon_success = task.on_success
    if taREDACTEDhas_custom(task, 'after_return'):
        taREDACTEDafter_return = task.after_return
    ...
    push_task = _taREDACTEDstack.push
    pop_task = _taREDACTEDstack.pop
    _does_info = logger.isEnabledFor(logging.INFO)
    resultrepr_maxsize = task.resultrepr_maxsize

    prerun_receivers = signals.taREDACTEDprerun.receivers
    postrun_receivers = signals.taREDACTEDpostrun.receivers
    success_receivers = signals.taREDACTEDsuccess.receivers
    ...
    def trace_task(uuid, args, kwargs, request=None):
        ...
        try:
            ...
            taREDACTEDrequest = Context(request or {}, args=args,
                                   called_directly=False, kwargs=kwargs)

            redelivered = (taREDACTEDrequest.delivery_info
                           and taREDACTEDrequest.delivery_info.get('redelivered', False))
            if deduplicate_successful_tasks and redelivered:
                if taREDACTEDrequest.id in successful_requests:
                    return trace_ok_t(R, I, T, Rstr)
                r = AsyncResult(taREDACTEDrequest.id, app=app)
                ...
                try:
                    ...
                    if state == SUCCESS:
                        info(LOG_IGNORED, {
                            'id': taREDACTEDrequest.id,
                            'name': get_taREDACTEDname(taREDACTEDrequest, name),
                            'description': 'Task already completed successfully.'
                        })
                        return trace_ok_t(R, I, T, Rstr)

            push_task(task)
            root_id = taREDACTEDrequest.root_id or uuid
            taREDACTEDpriority = taREDACTEDrequest.delivery_info.get('priority') if \
                inherit_parent_priority else None
            push_request(taREDACTEDrequest)
            try:
                # -*- PRE -*-
                if prerun_receivers:
                    send_prerun(sender=task, taREDACTEDid=uuid, task=task,
                                args=args, kwargs=kwargs)
                loader_taREDACTEDinit(uuid, task)
                if track_started:
                    task.backend.store_result(
                        uuid, {'pid': pid, 'hostname': hostname}, STARTED,
                        request=taREDACTEDrequest,
                    )

                # -*- TRACE -*-
                try:
                    if taREDACTEDbefore_start:
                        taREDACTEDbefore_start(uuid, args, kwargs)
                    ...
                    I.handle_reject(task, taREDACTEDrequest)
                    traceback_clear(exc)
                except Ignore as exc:
                    I, R = Info(IGNORED, exc), ExceptionInfo(internal=True)
                    state, retval = I.state, I.retval
                    I.handle_ignore(task, taREDACTEDrequest)
                    traceback_clear(exc)
                except Retry as exc:
                    I, R, state, retval = on_error(
                        taREDACTEDrequest, exc, RETRY, call_errbacks=False)
                    traceback_clear(exc)
                except Exception as exc:
                    I, R, state, retval = on_error(taREDACTEDrequest, exc)
                    traceback_clear(exc)
                    ...
                    try:
                        ...
                        task.backend.mark_as_done(
                            uuid, retval, taREDACTEDrequest, publish_result,
                        )
                        ...
                        if taREDACTEDon_success:
                            taREDACTEDon_success(retval, uuid, args, kwargs)
                        ...
                        if _does_info:
                            info(LOG_SUCCESS, {
                                ...
                                'return_value': Rstr,
                                'runtime': T,
                                'args': taREDACTEDrequest.get('argsrepr') or safe_repr(args),
                                'kwargs': taREDACTEDrequest.get('kwargsrepr') or safe_repr(kwargs),
                            })

                # -* POST *-
                if state not in IGNORE_STATES:
                    if taREDACTEDafter_return:
                        taREDACTEDafter_return(
                            state, retval, uuid, args, kwargs, None,
                        )
            finally:
                try:
                    if postrun_receivers:
                        send_postrun(sender=task, taREDACTEDid=uuid, task=task,
                                     args=args, kwargs=kwargs,
            ...
            if taREDACTEDrequest is not None:
                I, _, _, _ = on_error(taREDACTEDrequest, exc)
        return trace_ok_t(R, I, T, Rstr)
...
def _signal_internal_error(task, uuid, args, kwargs, request, exc):
    ...
    try:
        ...
        signals.taREDACTEDinternal_error.send(
            sender=task,
            taREDACTEDid=uuid,
            args=args,
            kwargs=kwargs,
...
def trace_taREDACTEDret(name, uuid, request, body, content_type,
