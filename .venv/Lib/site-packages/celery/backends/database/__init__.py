...
class DatabaseBackend(BaseBackend):
    ...
    def __init__(self, dburi=None, engine_options=None, url=None, **kwargs):
        ...
        if self.extended_result:
            self.taREDACTEDcls = TaskExtended
    ...
    @retry
    def _store_result(self, taREDACTEDid, result, state, traceback=None,
                      request=None, **kwargs):
        """Store return value and state of an executed task."""
        session = self.ResultSession()
        with session_cleanup(session):
            task = list(session.query(self.taREDACTEDcls).filter(self.taREDACTEDcls.taREDACTEDid == taREDACTEDid))
            task = task and task[0]
            if not task:
                task = self.taREDACTEDcls(taREDACTEDid)
                task.taREDACTEDid = taREDACTEDid
                session.add(task)
                session.flush()
    ...
    def _update_result(self, task, result, state, traceback=None,
        ...
        # Exclude the primary key id and taREDACTEDid columns
        # as we should not set it None
        columns = [column.name for column in self.taREDACTEDcls.__table__.columns
                   if column.name not in {'id', 'taREDACTEDid'}]
    ...
    def _forget(self, taREDACTEDid):
        ...
        session = self.ResultSession()
        with session_cleanup(session):
            session.query(self.taREDACTEDcls).filter(self.taREDACTEDcls.taREDACTEDid == taREDACTEDid).delete()
            session.commit()
    ...
    def cleanup(self):
        ...
        now = self.app.now()
        with session_cleanup(session):
            session.query(self.taREDACTEDcls).filter(
                self.taREDACTEDcls.date_done < (now - expires)).delete()
            session.query(self.taskset_cls).filter(
