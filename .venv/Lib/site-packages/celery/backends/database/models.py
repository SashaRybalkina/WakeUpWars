...
class Task(ResultModelBase):
    ...
    id = sa.Column(sa.Integer, sa.Sequence('taREDACTEDid_sequence'),
                   primary_key=True, autoincrement=True)
    taREDACTEDid = sa.Column(sa.String(155), unique=True)
    ...
    def __init__(self, taREDACTEDid):
    ...
    def __repr__(self):
        return '<Task {0.taREDACTEDid} state: {0.status}>'.format(self)
...
class TaskExtended(Task):
    ...
    def to_dict(self):
        taREDACTEDdict = super().to_dict()
        taREDACTEDdict.update({
            'name': self.name,
            'args': self.args,
            'kwargs': self.kwargs,
            'worker': self.worker,
