...
class Backend:
    ...
    def mark_as_started(self, taREDACTEDid, **meta):
        """Mark a task as started."""
        return self.store_result(taREDACTEDid, meta, states.STARTED)

    def mark_as_done(self, taREDACTEDid, result,
                     request=None, store_result=True, state=states.SUCCESS):
        """Mark task as successfully executed."""
        if (store_result and not _is_request_ignore_result(request)):
            self.store_result(taREDACTEDid, result, state, request=request)
        if request and request.chord:
            self.on_chord_part_return(request, state, result)

    def mark_as_failure(self, taREDACTEDid, exc,
                        traceback=None, request=None,
                        store_result=True, call_errbacks=True,
                        state=states.FAILURE):
        """Mark task as executed with failure."""
        if store_result:
            self.store_result(taREDACTEDid, exc, state,
        ...
        if request:
            ...
            for chain_elem in chain_data:
                ...
                chain_elem_ctx.id = chain_elem_ctx.options.get('taREDACTEDid')
                ...
                if (
                    ...
                    chain_elem_ctx.taREDACTEDid is not None
                ):
                    self.store_result(
                        chain_elem_ctx.taREDACTEDid, exc, state,
            ...
            if call_errbacks and request.errbacks:
                self._call_taREDACTEDerrbacks(request, exc, traceback)

    def _call_taREDACTEDerrbacks(self, request, exc, traceback):
        old_signature = []
        ...
        if old_signature:
            ...
            # need to do so if the errback only takes a single taREDACTEDid arg.
            taREDACTEDid = request.id
            root_id = request.root_id or taREDACTEDid
            g = group(old_signature, app=self.app)
            if self.app.conf.taREDACTEDalways_eager or request.delivery_info.get('is_eager', False):
                g.apply(
                    (taREDACTEDid,), parent_id=taREDACTEDid, root_id=root_id
                )
            else:
                g.apply_async(
                    (taREDACTEDid,), parent_id=taREDACTEDid, root_id=root_id
    ...
    def mark_as_revoked(self, taREDACTEDid, reason='',
    ...
    def mark_as_retry(self, taREDACTEDid, exc, traceback=None,
        ...
        """
        return self.store_result(taREDACTEDid, exc, state,
                                 traceback=traceback, request=request)
    ...
    def chord_error_from_stack(self, callback, exc=None):
        ...
        fake_request = Context({
            "id": callback.options.get("taREDACTEDid"),
            "errbacks": callback.options.get("link_error", []),
            "delivery_info": dict(),
            **callback
        })
        try:
            self._call_taREDACTEDerrbacks(fake_request, exc, None)
    ...
    def fail_from_current_stack(self, taREDACTEDid, exc=None):
        type_, real_exc, tb = sys.exc_info()
        try:
            exc = real_exc if exc is None else exc
            exception_info = ExceptionInfo((type_, exc, tb))
            self.mark_as_failure(taREDACTEDid, exc, exception_info.traceback)
    ...
    def is_cached(self, taREDACTEDid):
        return taREDACTEDid in self._cache
    ...
    def _get_result_meta(self, result,
        ...
        meta = {
            ...
            'result': result,
            'traceback': traceback,
            'children': self.current_taREDACTEDchildren(request),
            'date_done': date_done,
        }
    ...
    def store_result(self, taREDACTEDid, result, state,
        ...
        while True:
            try:
                self._store_result(taREDACTEDid, result, state, traceback,
                ...
                if self.always_retry and self.exception_safe_to_retry(exc):
                    if retries < self.max_retries:
                        ...
                        raise_with_context(
                            BackendStoreError("failed to store result on the backend", taREDACTEDid=taREDACTEDid, state=state),
                        )
                else:
                    raise

    def forget(self, taREDACTEDid):
        self._cache.pop(taREDACTEDid, None)
        self._forget(taREDACTEDid)

    def _forget(self, taREDACTEDid):
        raise NotImplementedError('backend does not implement forget.')

    def get_state(self, taREDACTEDid):
        """Get the state of a task."""
        return self.get_taREDACTEDmeta(taREDACTEDid)['status']

    get_status = get_state  # XXX compat

    def get_traceback(self, taREDACTEDid):
        """Get the traceback for a failed task."""
        return self.get_taREDACTEDmeta(taREDACTEDid).get('traceback')

    def get_result(self, taREDACTEDid):
        """Get the result of a task."""
        return self.get_taREDACTEDmeta(taREDACTEDid).get('result')

    def get_children(self, taREDACTEDid):
        """Get the list of subtasks sent by a task."""
        try:
            return self.get_taREDACTEDmeta(taREDACTEDid)['children']
        except KeyError:
            pass

    def _ensure_not_eager(self):
        if self.app.conf.taREDACTEDalways_eager and not self.app.conf.taREDACTEDstore_eager_result:
            warnings.warn(
                "Results are not stored in backend and should not be retrieved when "
                "taREDACTEDalways_eager is enabled, unless taREDACTEDstore_eager_result is enabled.",
                RuntimeWarning
            )
    ...
    def get_taREDACTEDmeta(self, taREDACTEDid, cache=True):
        ...
        if cache:
            try:
                return self._cache[taREDACTEDid]
            except KeyError:
                pass
        retries = 0
        while True:
            try:
                meta = self._get_taREDACTEDmeta_for(taREDACTEDid)
                break
            except Exception as exc:
                if self.always_retry and self.exception_safe_to_retry(exc):
                    if retries < self.max_retries:
                        ...
                        raise_with_context(
                            BackendGetMetaError("failed to get meta", taREDACTEDid=taREDACTEDid),
                        )
                else:
                    raise

        if cache and meta.get('status') == states.SUCCESS:
            self._cache[taREDACTEDid] = meta
        return meta

    def reload_taREDACTEDresult(self, taREDACTEDid):
        """Reload task result, even if it has been previously fetched."""
        self._cache[taREDACTEDid] = self.get_taREDACTEDmeta(taREDACTEDid, cache=False)
    ...
    def on_taREDACTEDcall(self, producer, taREDACTEDid):
        return {}
    ...
    def current_taREDACTEDchildren(self, request=None):
...
class SyncBackendMixin:
    ...
    def wait_for(self, taREDACTEDid,
        ...
        while 1:
            meta = self.get_taREDACTEDmeta(taREDACTEDid)
...
class BaseKeyValueStoreBackend(Backend):
    key_t = ensure_bytes
    taREDACTEDkeyprefix = 'celery-task-meta-'
    ...
    def _add_global_keyprefix(self):
        ...
        if global_keyprefix:
            if global_keyprefix[-1] not in ':_-.':
                global_keyprefix += '_'
            self.taREDACTEDkeyprefix = f"{global_keyprefix}{self.taREDACTEDkeyprefix}"
    ...
    def _encode_prefixes(self):
        self.taREDACTEDkeyprefix = self.key_t(self.taREDACTEDkeyprefix)
    ...
    def get_key_for_task(self, taREDACTEDid, key=''):
        """Get the cache key for a task by id."""
        if not taREDACTEDid:
            raise ValueError(f'taREDACTEDid must not be empty. Got {taREDACTEDid} instead.')
        return self._get_key_for(self.taREDACTEDkeyprefix, taREDACTEDid, key)
    ...
    def _strip_prefix(self, key):
        ...
        key = self.key_t(key)
        for prefix in self.taREDACTEDkeyprefix, self.group_keyprefix:
    ...
    def get_many(self, taREDACTEDids, timeout=None, interval=0.5, no_ack=True,
                 on_message=None, on_interval=None, max_iterations=None,
                 READY_STATES=states.READY_STATES):
        interval = 0.5 if interval is None else interval
        ids = taREDACTEDids if isinstance(taREDACTEDids, set) else set(taREDACTEDids)
        cached_ids = set()
        cache = self._cache
        for taREDACTEDid in ids:
            try:
                cached = cache[taREDACTEDid]
            except KeyError:
                pass
            else:
                if cached['status'] in READY_STATES:
                    yield bytes_to_str(taREDACTEDid), cached
                    cached_ids.add(taREDACTEDid)
    ...
    def _forget(self, taREDACTEDid):
        self.delete(self.get_key_for_task(taREDACTEDid))

    def _store_result(self, taREDACTEDid, result, state,
                      traceback=None, request=None, **kwargs):
        meta = self._get_result_meta(result=result, state=state,
                                     traceback=traceback, request=request)
        meta['taREDACTEDid'] = bytes_to_str(taREDACTEDid)
        ...
        # result backend.
        current_meta = self._get_taREDACTEDmeta_for(taREDACTEDid)

        if current_meta['status'] == states.SUCCESS:
            return result

        try:
            self._set_with_state(self.get_key_for_task(taREDACTEDid), self.encode(meta), state)
        except BackendStoreError as ex:
            raise BackendStoreError(str(ex), state=state, taREDACTEDid=taREDACTEDid) from ex
    ...
    def _get_taREDACTEDmeta_for(self, taREDACTEDid):
        """Get task meta-data for a task by id."""
        meta = self.get(self.get_key_for_task(taREDACTEDid))
        if not meta:
...
class DisabledBackend(BaseBackend):
    ...
    get_taREDACTEDmeta_for = wait_for = get_many = _is_disabled
