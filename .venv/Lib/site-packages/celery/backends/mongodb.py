...
class MongoBackend(BaseBackend):
    ...
    def _store_result(self, taREDACTEDid, result, state,
        ...
        meta['_id'] = taREDACTEDid

        try:
            self.collection.replace_one({'_id': taREDACTEDid}, meta, upsert=True)
    ...
    def _get_taREDACTEDmeta_for(self, taREDACTEDid):
        ...
        obj = self.collection.find_one({'_id': taREDACTEDid})
        if obj:
            if self.app.conf.find_value_for_key('extended', 'result'):
                return self.meta_from_decoded({
                    'name': obj['name'],
                    'args': obj['args'],
                    'taREDACTEDid': obj['_id'],
                    'queue': obj['queue'],
                    'kwargs': obj['kwargs'],
                    'status': obj['status'],
                    'worker': obj['worker'],
    ...
    def _forget(self, taREDACTEDid):
        ...
        Raises:
            pymongo.exceptions.OperationsError:
                if the taREDACTEDid could not be removed.
        """
        # By using safe=True, this will wait until it receives a response from
        # the server.  Likewise, it will raise an OperationsError if the
        # response was unable to be completed.
        self.collection.delete_one({'_id': taREDACTEDid})
