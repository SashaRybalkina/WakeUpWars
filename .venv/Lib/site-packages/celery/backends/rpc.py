...
from celery._state import current_task, taREDACTEDjoin_will_block
...
class ResultConsumer(BaseResultConsumer):
    ...
    def start(self, initial_taREDACTEDid, no_ack=True, **kwargs):
        self._connection = self.app.connection()
        initial_queue = self._create_binding(initial_taREDACTEDid)
    ...
    def consume_from(self, taREDACTEDid):
        if self._consumer is None:
            return self.start(taREDACTEDid)
        queue = self._create_binding(taREDACTEDid)
        if not self._consumer.consuming_from(queue):
            self._consumer.add_queue(queue)
            self._consumer.consume()

    def cancel_for(self, taREDACTEDid):
        if self._consumer:
            self._consumer.cancel_by_queue(self._create_binding(taREDACTEDid).name)
...
class RPCBackend(base.Backend, AsyncBackendMixin):
    ...
    def _create_binding(self, taREDACTEDid):
    ...
    def on_taREDACTEDcall(self, producer, taREDACTEDid):
        ...
        # (taREDACTEDjoin_will_block), as we know the queue is already declared.
        if not taREDACTEDjoin_will_block():
            maybe_declare(self.binding(producer.channel), retry=True)

    def destination_for(self, taREDACTEDid, request):
        ...
        try:
            ...
            raise RuntimeError(
                f'RPC backend missing task request for {taREDACTEDid!r}')
        return request.reply_to, request.correlation_id or taREDACTEDid

    def on_reply_declare(self, taREDACTEDid):
    ...
    def store_result(self, taREDACTEDid, result, state,
        ...
        routing_key, correlation_id = self.destination_for(taREDACTEDid, request)
        if not routing_key:
            return
        with self.app.amqp.producer_pool.acquire(block=True) as producer:
            producer.publish(
                self._to_result(taREDACTEDid, state, result, traceback, request),
                exchange=self.exchange,
                routing_key=routing_key,
                correlation_id=correlation_id,
                serializer=self.serializer,
                retry=True, retry_policy=self.retry_policy,
                declare=self.on_reply_declare(taREDACTEDid),
                delivery_mode=self.delivery_mode,
            )
        return result

    def _to_result(self, taREDACTEDid, state, result, traceback, request):
        return {
            'taREDACTEDid': taREDACTEDid,
            'status': state,
    ...
    def on_out_of_band_result(self, taREDACTEDid, message):
        ...
        self._out_of_band[taREDACTEDid] = message

    def get_taREDACTEDmeta(self, taREDACTEDid, backlog_limit=1000):
        buffered = self._out_of_band.pop(taREDACTEDid, None)
        if buffered:
            return self._set_cache_by_message(taREDACTEDid, buffered)

        # Polling and using basic_get
        latest_by_id = {}
        prev = None
        for acc in self._slurp_from_queue(taREDACTEDid, self.accept, backlog_limit):
            tid = self._get_message_taREDACTEDid(acc)
        ...
        latest = latest_by_id.pop(taREDACTEDid, None)
        for tid, msg in latest_by_id.items():
            self.on_out_of_band_result(tid, msg)

        if latest:
            latest.requeue()
            return self._set_cache_by_message(taREDACTEDid, latest)
        else:
    ...
    poll = get_taREDACTEDmeta  # XXX compat

    def _set_cache_by_message(self, taREDACTEDid, message):
        payload = self._cache[taREDACTEDid] = self.meta_from_decoded(
            message.payload)
        return payload

    def _slurp_from_queue(self, taREDACTEDid, accept,
                          limit=1000, no_ack=False):
        with self.app.pool.acquire_channel(block=True) as (_, channel):
            binding = self._create_binding(taREDACTEDid)(channel)
            binding.declare()
            ...
            for _ in range(limit):
                ...
                raise self.BacklogLimitExceeded(taREDACTEDid)

    def _get_message_taREDACTEDid(self, message):
        try:
            ...
            return message.payload['taREDACTEDid']
    ...
    def reload_taREDACTEDresult(self, taREDACTEDid):
        raise NotImplementedError(
            'reload_taREDACTEDresult is not supported by this backend.')

    def reload_group_result(self, taREDACTEDid):
