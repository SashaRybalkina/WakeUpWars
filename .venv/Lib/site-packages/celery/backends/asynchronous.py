...
class AsyncBackendMixin:
    ...
    def _add_pending_result(self, taREDACTEDid, result, weak=False):
        concrete, weak_ = self._pending_results
        if taREDACTEDid not in weak_ and result.id not in concrete:
            (weak_ if weak else concrete)[taREDACTEDid] = result
            self.result_consumer.consume_from(taREDACTEDid)
    ...
    def _remove_pending_result(self, taREDACTEDid):
        for mapping in self._pending_results:
            mapping.pop(taREDACTEDid, None)
...
class BaseResultConsumer:
    ...
    def start(self, initial_taREDACTEDid, **kwargs):
    ...
    def _get_pending_result(self, taREDACTEDid):
        for mapping in self._pending_results:
            try:
                return mapping[taREDACTEDid]
            except KeyError:
                pass
        raise KeyError(taREDACTEDid)

    def on_state_change(self, meta, message):
        if self.on_message:
            self.on_message(meta)
        if meta['status'] in states.READY_STATES:
            taREDACTEDid = meta['taREDACTEDid']
            try:
                result = self._get_pending_result(taREDACTEDid)
            except KeyError:
                # send to buffer in case we received this result
                # before it was added to _pending_results.
                self._pending_messages.put(taREDACTEDid, meta)
            else:
