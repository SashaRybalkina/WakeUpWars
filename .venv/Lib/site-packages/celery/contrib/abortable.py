...
.. code-block:: python
    ...
    from celery.utils.log import get_taREDACTEDlogger
...
class AbortableTask(Task):
    ...
    def AsyncResult(self, taREDACTEDid):
        """Return the accompanying AbortableAsyncResult instance."""
        return AbortableAsyncResult(taREDACTEDid, backend=self.backend)
    ...
    def is_aborted(self, **kwargs):
        ...
        Always return :const:`False` in case the `taREDACTEDid` parameter
        ...
        """
        taREDACTEDid = kwargs.get('taREDACTEDid', self.request.id)
        result = self.AsyncResult(taREDACTEDid)
