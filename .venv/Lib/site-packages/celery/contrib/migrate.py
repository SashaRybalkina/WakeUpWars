...
__all__ = (
    ...
    'migrate_tasks', 'move', 'taREDACTEDid_eq', 'taREDACTEDid_in',
    'start_filter', 'move_taREDACTEDby_id', 'move_by_idmap',
...
def move(predicate, connection=None, exchange=None, routing_key=None,
    ...
    Arguments:
        ...
        source: List[Union[str, kombu.Queue]]: Optional list of source
            ...
            in :setting:`taREDACTEDqueues`).  This list can also contain
    ...
    To demonstrate, the :func:`move_taREDACTEDby_id` operation can be implemented
    like this:
...
def taREDACTEDid_eq(taREDACTEDid, body, message):
    """Return true if task id equals taREDACTEDid'."""
    return body['id'] == taREDACTEDid


def taREDACTEDid_in(ids, body, message):
...
def move_taREDACTEDby_id(taREDACTEDid, dest, **kwargs):
    """Find a task by id and move it to another queue.

    Arguments:
        taREDACTEDid (str): Id of task to find and move.
    ...
    """
    return move_by_idmap({taREDACTEDid: dest}, **kwargs)


def move_by_idmap(map, **kwargs):
    """Move tasks by matching from a ``taREDACTEDid: queue`` mapping.
    ...
    Example:
        ...
        ...   queues=['hipri'])
    """
    def taREDACTEDid_in_map(body, message):
    ...
    return move(taREDACTEDid_in_map, limit=len(map), **kwargs)


def move_by_taskmap(map, **kwargs):
    """Move tasks by matching from a ``taREDACTEDname: queue`` mapping.
    ...
    Example:
        ...
        ... })
    """
    def taREDACTEDname_in_map(body, message):
        return map.get(body['task'])  # <- name of task

    return move(taREDACTEDname_in_map, **kwargs)
...
move_direct_by_id = partial(move_taREDACTEDby_id, transform=worker_direct)
