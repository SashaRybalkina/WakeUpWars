...
class ManagerMixin:
    ...
    def join(self, r, propagate=False, max_retries=10, **kwargs):
        ...
        def on_result(taREDACTEDid, value):
            received.append(taREDACTEDid)
    ...
    def query_taREDACTEDstates(self, ids, timeout=0.5):
        states = defaultdict(set)
        for hostname, reply in self.query_tasks(ids, timeout=timeout):
            for taREDACTEDid, (state, _) in reply.items():
                states[state].add(taREDACTEDid)
        return states

    def assert_accepted(self, ids, interval=0.5,
                        desc='waiting for tasks to be accepted', **policy):
        return self.assert_taREDACTEDworker_state(
    ...
    def assert_result_tasks_in_progress_or_completed(
        ...
        return self.assert_taREDACTEDstate_from_result(
            self.is_result_taREDACTEDin_progress,
            async_results,
            interval=interval, desc=desc, **policy
        )

    def assert_taREDACTEDstate_from_result(self, fun, results,
                                      interval=0.5, **policy):
        return self.wait_for(
    ...
    @staticmethod
    def is_result_taREDACTEDin_progress(results, **kwargs):
        possible_states = (states.STARTED, states.SUCCESS)
        return all(result.state in possible_states for result in results)

    def assert_taREDACTEDworker_state(self, fun, ids, interval=0.5, **policy):
        return self.wait_for(
    ...
    def _ids_matches_state(self, expected_states, ids, timeout=0.5):
        states = self.query_taREDACTEDstates(ids, timeout=timeout)
        return all(
        ...
        with self.app.connection() as connection:
            ...
            while True:
                try:
                    ...
                    # test_security_taREDACTEDdone may trigger this exception
                    break
                if count == 0:
                    break
