...
then you can change the ``celery_taREDACTEDprefix`` configuration value:

.. code-block:: python

    celery_taREDACTEDprefix = '(task)'  # < default
...
class TaskDirective(PyFunction):
    ...
    def get_signature_prefix(self, sig):
        return [nodes.Text(self.env.config.celery_taREDACTEDprefix)]
...
def setup(app):
    ...
    app.add_config_value('celery_taREDACTEDprefix', '(task)', True)
