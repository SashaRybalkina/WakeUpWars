...
def _find_requests_by_id(ids,
    ...
    for taREDACTEDid in ids:
        try:
            yield get_request(taREDACTEDid)
        except KeyError:
            pass
...
@control_command(
    variadic='taREDACTEDid',
    signature='[id1 [id2 [... [idN]]]]',
)
def revoke(state, taREDACTEDid, terminate=False, signal=None, **kwargs):
    ...
    taREDACTEDids, taREDACTEDid = set(maybe_list(taREDACTEDid) or []), None
    taREDACTEDids = _revoke(state, taREDACTEDids, terminate, signal, **kwargs)
    if isinstance(taREDACTEDids, dict) and 'ok' in taREDACTEDids:
        return taREDACTEDids
    return ok(f'tasks {taREDACTEDids} flagged as revoked')
...
def _revoke(state, taREDACTEDids, terminate=False, signal=None, **kwargs):
    size = len(taREDACTEDids)
    terminated = set()

    worker_state.revoked.update(taREDACTEDids)
    if terminate:
        signum = _signals.signum(signal or TERM_SIGNAME)
        for request in _find_requests_by_id(taREDACTEDids):
...
def terminate(state, signal, taREDACTEDid, **kwargs):
    """Terminate task by task id (or list of ids)."""
    return revoke(state, taREDACTEDid, terminate=True, signal=signal)


@control_command(
    args=[('taREDACTEDname', str), ('rate_limit', str)],
    signature='<taREDACTEDname> <rate_limit (e.g., 5/s | 5/m | 5/h)>',
)
def rate_limit(state, taREDACTEDname, rate_limit, **kwargs):
    ...
    Arguments:
        taREDACTEDname (str): Type of task to set rate limit for.
    ...
    try:
        state.app.tasks[taREDACTEDname].rate_limit = rate_limit
    except KeyError:
        logger.error('Rate limit attempt for unknown task %s',
                     taREDACTEDname, exc_info=True)
        return nok('unknown task')

    state.consumer.reset_rate_limits()

    if not rate_limit:
        logger.info('Rate limits disabled for tasks of type %s', taREDACTEDname)
        return ok('rate limit disabled successfully')

    logger.info('New rate limit for tasks of type %s: %s.',
                taREDACTEDname, rate_limit)
    return ok('new rate limit set successfully')


@control_command(
    args=[('taREDACTEDname', str), ('soft', float), ('hard', float)],
    signature='<taREDACTEDname> <soft_secs> [hard_secs]',
)
def time_limit(state, taREDACTEDname=None, hard=None, soft=None, **kwargs):
    """Tell worker(s) to modify the time limit for task by type.

    Arguments:
        taREDACTEDname (str): Name of task to change.
        hard (float): Hard time limit.
        soft (float): Soft time limit.
    """
    try:
        task = state.app.tasks[taREDACTEDname]
    except KeyError:
...
def add_consumer(state, queue, exchange=None, exchange_type=None,
    ...
    state.consumer.call_soon(
        state.consumer.add_taREDACTEDqueue,
...
def cancel_consumer(state, queue, **_):
    ...
    state.consumer.call_soon(
        state.consumer.cancel_taREDACTEDqueue, queue,
    )
...
def active_queues(state):
    ...
    if state.consumer.taREDACTEDconsumer:
        return [dict(queue.as_dict(recurse=True))
                for queue in state.consumer.taREDACTEDconsumer.queues]
    return []
