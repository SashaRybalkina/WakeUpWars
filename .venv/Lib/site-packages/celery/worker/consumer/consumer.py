...
from celery.worker.state import active_requests, maybe_shutdown, requests, reserved_requests, taREDACTEDreserved
...
class Consumer:
    ...
    def __init__(self, on_taREDACTEDrequest,
        ...
        self._limit_order = 0
        self.on_taREDACTEDrequest = on_taREDACTEDrequest
        self.on_taREDACTEDmessage = set()
        ...
        self.taREDACTEDbuckets = defaultdict(lambda: None)
    ...
    def _limit_move_to_pool(self, request):
        taREDACTEDreserved(request)
        self.on_taREDACTEDrequest(request)
    ...
    def loop_args(self):
        return (self, self.connection, self.taREDACTEDconsumer,
    ...
    def on_close(self):
        ...
        if self.timer:
            self.timer.clear()
        for bucket in self.taREDACTEDbuckets.values():
            if bucket:
    ...
    def add_taREDACTEDqueue(self, queue, exchange=None, exchange_type=None,
                       routing_key=None, **options):
        cset = self.taREDACTEDconsumer
        queues = self.app.amqp.queues
        # Must use in' here, as __missing__ will automatically
        # create queues when :setting:`taREDACTEDcreate_missing_queues` is enabled.
        # (Issue #1079)
        if queue in queues:
            q = queues[queue]
        else:
    ...
    def cancel_taREDACTEDqueue(self, queue):
        info('Canceling queue %s', queue)
        self.app.amqp.queues.deselect(queue)
        self.taREDACTEDconsumer.cancel_by_queue(queue)

    def apply_eta_task(self, task):
        """Method called by the timer to apply a task with an ETA/countdown."""
        taREDACTEDreserved(task)
        self.on_taREDACTEDrequest(task)
        self.qos.decrement_eventually()
    ...
    def on_unknown_message(self, body, message):
        ...
        signals.taREDACTEDrejected.send(sender=self, message=message, exc=None)
    ...
    def on_unknown_task(self, body, message, exc):
        ...
        signals.taREDACTEDunknown.send(
    ...
    def on_invalid_task(self, body, message, exc):
        error(INVALID_TASK_ERROR, exc, dump_body(message, body),
              exc_info=True)
        message.reject_log_error(logger, self.connection_errors)
        signals.taREDACTEDrejected.send(sender=self, message=message, exc=exc)
    ...
    def create_taREDACTEDhandler(self, promise=promise):
        strategies = self.strategies
        on_unknown_message = self.on_unknown_message
        on_unknown_task = self.on_unknown_task
        on_invalid_task = self.on_invalid_task
        callbacks = self.on_taREDACTEDmessage
        call_soon = self.call_soon

        def on_taREDACTEDreceived(message):
