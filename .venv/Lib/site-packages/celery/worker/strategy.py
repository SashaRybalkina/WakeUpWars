...
from .state import taREDACTEDreserved
...
def default(task, app, consumer,
            info=logger.info, error=logger.error, taREDACTEDreserved=taREDACTEDreserved,
    ...
    taREDACTEDsends_events = events and task.send_events

    call_at = consumer.timer.call_at
    apply_eta_task = consumer.apply_eta_task
    rate_limits_enabled = not consumer.disable_rate_limits
    get_bucket = consumer.taREDACTEDbuckets.__getitem__
    handle = consumer.on_taREDACTEDrequest
    ...
    def taREDACTEDmessage_handler(message, body, ack, reject, callbacks,
        ...
        signals.taREDACTEDreceived.send(sender=consumer, request=req)

        if taREDACTEDsends_events:
            send_event(
                'task-received',
        ...
        if eta:
            ...
            return taREDACTEDmessage_handler
        if bucket:
            return limit_task(req, bucket, 1)

        taREDACTEDreserved(req)
        if callbacks:
