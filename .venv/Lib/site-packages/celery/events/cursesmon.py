...
STATUS_SCREEN = """\
events: {s.event_count} tasks:{s.taREDACTEDcount} workers:{w_alive}/{w_all}
"""
...
class CursesMonitor:  # pragma: no cover
    ...
    def format_row(self, uuid, task, worker, timestamp, state):
        ...
        taREDACTEDwidth = int(ceil(detail_width / 2.0))
        worker_width = detail_width - taREDACTEDwidth - 1

        uuid = abbr(uuid, uuid_width).ljust(uuid_width)
        worker = abbr(worker, worker_width).ljust(worker_width)
        task = abbrtask(task, taREDACTEDwidth).ljust(taREDACTEDwidth)
    ...
    def display_taREDACTEDrow(self, lineno, task):
        ...
        with self.lock:
            ...
            if tasks:
                for row, (_, task) in enumerate(tasks):
                    ...
                    if task.uuid:
                        lineno = next(y)
                    self.display_taREDACTEDrow(lineno, task)
