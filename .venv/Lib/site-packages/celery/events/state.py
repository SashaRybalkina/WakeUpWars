...
R_STATE = '<State: events={0.event_count} tasks={0.taREDACTEDcount}>'
...
class Task:
    ...
    def __init__(self, uuid=None, cluster_state=None, children=None, **kwargs):
        ...
        if self.cluster_state is not None:
            self.children = WeakSet(
                self.cluster_state.tasks.get(taREDACTEDid)
                for taREDACTEDid in children or ()
                if taREDACTEDid in self.cluster_state.tasks
            )
        else:
            self.children = WeakSet()
    ...
    def event(self, type_, timestamp=None, local_received=None, fields=None,
              ...
              taREDACTEDevent_to_state=TASK_EVENT_TO_STATE.get, RETRY=states.RETRY):
        fields = fields or {}

        # using .get is faster than catching KeyError in this case.
        state = taREDACTEDevent_to_state(type_)
...
class State:
    ...
    def __init__(self, callback=None,
        ...
        self.tasks_by_type.update(
            _deserialize_TaREDACTEDWeakSet_Mapping(tasks_by_type, self.tasks))
        ...
        self.tasks_by_worker.update(
            _deserialize_TaREDACTEDWeakSet_Mapping(tasks_by_worker, self.tasks))
    ...
    def taREDACTEDevent(self, type_, fields):
    ...
    def _create_dispatcher(self):
        ...
        get_taREDACTEDby_type_set = self.tasks_by_type.__getitem__
        get_taREDACTEDby_worker_set = self.tasks_by_worker.__getitem__
        ...
        def _event(event,
            ...
            if group == 'worker':
                ...
                try:
                    task, taREDACTEDcreated = get_task(uuid), False
                except KeyError:
                    task = tasks[uuid] = Task(uuid, cluster_state=self)
                    taREDACTEDcreated = True
                if is_client_event:
                ...
                if subject == 'received':
                    self.taREDACTEDcount += 1
                task.event(subject, timestamp, local_received, event)
                taREDACTEDname = task.name
                if taREDACTEDname is not None:
                    add_type(taREDACTEDname)
                    if taREDACTEDcreated:  # add to tasks_by_type index
                        get_taREDACTEDby_type_set(taREDACTEDname).add(task)
                        get_taREDACTEDby_worker_set(hostname).add(task)
                if task.parent_id:
                    try:
                        parent_task = self.tasks[task.parent_id]
                    except KeyError:
                        self._add_pending_taREDACTEDchild(task)
                    else:
                ...
                return (task, taREDACTEDcreated), subject
        return _event

    def _add_pending_taREDACTEDchild(self, task):
        try:
    ...
    def __reduce__(self):
        return self.__class__, (
            ...
            _serialize_TaREDACTEDWeakSet_Mapping(self.tasks_by_type),
            _serialize_TaREDACTEDWeakSet_Mapping(self.tasks_by_worker),
        )


def _serialize_TaREDACTEDWeakSet_Mapping(mapping):
    return {name: [t.id for t in tasks] for name, tasks in mapping.items()}


def _deserialize_TaREDACTEDWeakSet_Mapping(mapping, tasks):
    mapping = mapping or {}
