...
from ._state import _set_taREDACTEDjoin_will_block, taREDACTEDjoin_will_block
...
def assert_will_not_block():
    if taREDACTEDjoin_will_block():
        raise RuntimeError(E_WOULDBLOCK)


@contextmanager
def allow_join_result():
    reset_value = taREDACTEDjoin_will_block()
    _set_taREDACTEDjoin_will_block(False)
    try:
        yield
    finally:
        _set_taREDACTEDjoin_will_block(reset_value)


@contextmanager
def denied_join_result():
    reset_value = taREDACTEDjoin_will_block()
    _set_taREDACTEDjoin_will_block(True)
    try:
        yield
    finally:
        _set_taREDACTEDjoin_will_block(reset_value)
...
class AsyncResult(ResultBase):
    ...
    def __init__(self, id, backend=None,
                 taREDACTEDname=None,            # deprecated
    ...
    def maybe_throw(self, propagate=True, callback=None):
        cache = self._get_taREDACTEDmeta() if self._cache is None else self._cache
        state, value, tb = (
    ...
    def _to_remote_traceback(self, tb):
        if tb and tblib is not None and self.app.conf.taREDACTEDremote_tracebacks:
    ...
    @property
    def children(self):
        return self._get_taREDACTEDmeta().get('children')
    ...
    def _get_taREDACTEDmeta(self):
        if self._cache is None:
            return self._maybe_set_cache(self.backend.get_taREDACTEDmeta(self.id))
        return self._cache

    def _iter_meta(self, **kwargs):
        return iter([self._get_taREDACTEDmeta()])
    ...
    def result(self):
        ...
        Note:
            ...
            instance.
        """
        return self._get_taREDACTEDmeta()['result']
    info = result

    @property
    def traceback(self):
        """Get the traceback of a failed task."""
        return self._get_taREDACTEDmeta().get('traceback')
    ...
    def state(self):
        ...
        """
        return self._get_taREDACTEDmeta()['status']
    status = state  # XXX compat

    @property
    def taREDACTEDid(self):
        """Compat. alias to :attr:`id`."""
        return self.id

    @taREDACTEDid.setter
    def taREDACTEDid(self, id):
        self.id = id

    @property
    def name(self):
        return self._get_taREDACTEDmeta().get('name')

    @property
    def args(self):
        return self._get_taREDACTEDmeta().get('args')

    @property
    def kwargs(self):
        return self._get_taREDACTEDmeta().get('kwargs')

    @property
    def worker(self):
        return self._get_taREDACTEDmeta().get('worker')

    @property
    def date_done(self):
        """UTC date and time."""
        date_done = self._get_taREDACTEDmeta().get('date_done')
    ...
    @property
    def retries(self):
        return self._get_taREDACTEDmeta().get('retries')

    @property
    def queue(self):
        return self._get_taREDACTEDmeta().get('queue')
...
class ResultSet(ResultBase):
    ...
    def join(self, timeout=None, propagate=True, interval=0.5,
        ...
        Arguments:
            ...
            callback (Callable): Optional callback to be called for every
                result received.  Must have signature ``(taREDACTEDid, value)``
                ...
                ``result = app.AsyncResult(taREDACTEDid)`` (both will
    ...
    def join_native(self, timeout=None, propagate=True,
        ...
        for taREDACTEDid, meta in self.iter_native(timeout, interval, no_ack,
            ...
            if isinstance(meta, list):
                ...
                if propagate and meta['status'] in states.PROPAGATE_STATES:
                    raise value
            if callback:
                callback(taREDACTEDid, value)
            else:
                acc[order_index[taREDACTEDid]] = value
        return acc
