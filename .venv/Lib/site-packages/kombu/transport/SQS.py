...
class QoS(virtual.QoS):
    ...
    def apply_backoff_policy(self, routing_key, delivery_tag,
        ...
        taREDACTEDname, number_of_retries = \
            self.extract_taREDACTEDname_and_number_of_retries(delivery_tag)
        if not taREDACTEDname or not number_of_retries:
            return None
        policy_value = backoff_policy.get(number_of_retries)
        if taREDACTEDname in backoff_tasks and policy_value is not None:
    ...
    def extract_taREDACTEDname_and_number_of_retries(self, delivery_tag):
        message = self._delivered[delivery_tag]
        message_headers = message.headers
        taREDACTEDname = message_headers['task']
        number_of_retries = int(
            message.properties['delivery_info']['sqs_message']
                              ['Attributes']['ApproximateReceiveCount'])
        return taREDACTEDname, number_of_retries
