...
def taREDACTEDqueue(
    ...
    """
    return _get_functions_service(app).taREDACTEDqueue(function_name, extension_id)
...
class _FunctionsService:
    ...
    def taREDACTEDqueue(self, function_name: str, extension_id: Optional[str] = None) -> TaskQueue:
...
class TaskQueue:
    ...
    def enqueue(self, taREDACTEDdata: Any, opts: Optional[TaskOptions] = None) -> str:
        ...
        Args:
            taREDACTEDdata: The data payload of the task.
        ...
        """
        task = self._validate_taREDACTEDoptions(taREDACTEDdata, self._resource, opts)
        service_url = self._get_url(self._resource, _CLOUD_TASKS_API_URL_FORMAT)
        taREDACTEDpayload = self._update_taREDACTEDpayload(task, self._resource, self._extension_id)
        try:
            resp = self._http_client.body(
                'post',
                url=service_url,
                headers=_FUNCTIONS_HEADERS,
                json={'task': taREDACTEDpayload.__dict__}
            )
            taREDACTEDname = resp.get('name', None)
            taREDACTEDresource = \
                self._parse_resource_name(taREDACTEDname, f'queues/{self._resource.resource_id}/tasks')
            return taREDACTEDresource.resource_id
        except requests.exceptions.RequestException as error:
            raise _FunctionsService.handle_functions_error(error)

    def delete(self, taREDACTEDid: str) -> None:
        ...
        Args:
            taREDACTEDid: The ID of the task relative to this queue.
        ...
        Raises:
            ...
            ValueError: If the input arguments are invalid.
        """
        _Validators.check_non_empty_string('taREDACTEDid', taREDACTEDid)
        service_url = self._get_url(self._resource, _CLOUD_TASKS_API_URL_FORMAT + f'/{taREDACTEDid}')
        try:
    ...
    def _validate_taREDACTEDoptions(
            self,
            data: Any,
            resource: Resource,
        ...
        task = Task(http_request=taREDACTEDhttp_request)
        ...
        if opts is not None:
            ...
            if opts.taREDACTEDid is not None:
                if not _Validators.is_taREDACTEDid(opts.taREDACTEDid):
                    raise ValueError(
                        'taREDACTEDid can contain only letters ([A-Za-z]), numbers ([0-9]), hyphens (-)'
                        ', or underscores (_). The maximum length is 500 characters.')
                task.name = self._get_url(
                    resource, _CLOUD_TASKS_API_RESOURCE_PATH + f'/{opts.taREDACTEDid}')
    ...
    def _update_taREDACTEDpayload(self, task: Task, resource: Resource, extension_id: str) -> Task:
...
class _Validators:
    ...
    @classmethod
    def is_taREDACTEDid(cls, taREDACTEDid: Any):
        """Checks if given value is a valid task id."""
        reg = '^[A-Za-z0-9_-]+$'
        if re.match(reg, taREDACTEDid) is not None and len(taREDACTEDid) <= 500:
            return True
        return False
...
class TaskOptions:
    ...
    Args:
        ...
        taREDACTEDid: The ID to use for the enqueued task. If not provided, one will be automatically
            generated.
    ...
    taREDACTEDid: Optional[str] = None
