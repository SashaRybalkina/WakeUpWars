...
class Pool:
    ...
    def __init__(self, processes=None, initializer=None, initargs=(),
        ...
        self._taREDACTEDhandler = self.TaskHandler(self._taskqueue,
                                              self._quick_put,
                                              self._outqueue,
                                              self._pool,
                                              self._cache)
        if threads:
            self._taREDACTEDhandler.start()
        ...
        self._terminate = Finalize(
            ...
            args=(self._taskqueue, self._inqueue, self._outqueue,
                  self._pool, self._worker_handler, self._taREDACTEDhandler,
    ...
    def _help_stuff_finish_args(self):
        return self._inqueue, self._taREDACTEDhandler, self._pool
    ...
    def imap(self, func, iterable, chunksize=1, lost_worker_timeout=None):
        ...
        if chunksize == 1:
            ...
            assert chunksize > 1
            taREDACTEDbatches = Pool._get_tasks(func, iterable, chunksize)
            result = IMapIterator(self._cache,
                                  lost_worker_timeout=lost_worker_timeout)
            self._taskqueue.put((
                ((TASK, (result._job, i, mapstar, (x,), {}))
                 for i, x in enumerate(taREDACTEDbatches)),
                result._set_length,
            ))
    ...
    def _map_async(self, func, iterable, mapper, chunksize=None,
        ...
        self._taskqueue.put((((TASK, (result._job, i, mapper, (x,), {}))
                              for i, x in enumerate(taREDACTEDbatches)), None))
        return result
    ...
    @staticmethod
    def _stop_taREDACTEDhandler(taREDACTEDhandler):
        stop_if_not_current(taREDACTEDhandler)
    ...
    def join(self):
        ...
        self._stop_taREDACTEDhandler(self._taREDACTEDhandler)
    ...
    @staticmethod
    def _help_stuff_finish(inqueue, taREDACTEDhandler, _pool):
        # taREDACTEDhandler may be blocked trying to put items on inqueue
        debug('removing tasks from inqueue until task handler finished')
        inqueue._rlock.acquire()
        while taREDACTEDhandler.is_alive() and inqueue._reader.poll():
    ...
    def _terminate_pool(cls, taskqueue, inqueue, outqueue, pool,
                        worker_handler, taREDACTEDhandler,
        ...
        debug('joining task handler')
        cls._stop_taREDACTEDhandler(taREDACTEDhandler)
...
class ThreadPool(Pool):
    ...
    @staticmethod
    def _help_stuff_finish(inqueue, taREDACTEDhandler, pool):
