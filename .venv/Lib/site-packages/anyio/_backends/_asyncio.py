...
def find_root_task() -> asyncio.Task:
    ...
    state = _taREDACTEDstates.get(task)
    if state:
...
def _taREDACTEDstarted(task: asyncio.Task) -> bool:
...
class CancelScope(BaseCancelScope):
    ...
    def __enter__(self) -> CancelScope:
        ...
        try:
            taREDACTEDstate = _taREDACTEDstates[host_task]
        except KeyError:
            taREDACTEDstate = TaskState(None, self)
            _taREDACTEDstates[host_task] = taREDACTEDstate
        else:
            self._parent_scope = taREDACTEDstate.cancel_scope
            taREDACTEDstate.cancel_scope = self
    ...
    def __exit__(
        ...
        host_taREDACTEDstate = _taREDACTEDstates.get(self._host_task)
        if host_taREDACTEDstate is None or host_taREDACTEDstate.cancel_scope is not self:
            raise RuntimeError(
        ...
        try:
            ...
            host_taREDACTEDstate.cancel_scope = self._parent_scope
    ...
    def _deliver_cancellation(self, origin: CancelScope) -> bool:
        ...
        for task in self._tasks:
            ...
            if task is not current and (task is self._host_task or _taREDACTEDstarted(task)):
...
_taREDACTEDstates: WeakKeyDictionary[asyncio.Task, TaskState] = WeakKeyDictionary()
...
class _AsyncioTaskStatus(abc.TaskStatus):
    ...
    def started(self, value: T_contra | None = None) -> None:
        ...
        task = cast(asyncio.Task, current_task())
        _taREDACTEDstates[task].parent_id = self._parent_id


if sys.version_info >= (3, 12):
    _eager_taREDACTEDfactory_code: CodeType | None = asyncio.eager_taREDACTEDfactory.__code__
else:
    _eager_taREDACTEDfactory_code = None
...
class TaskGroup(abc.TaskGroup):
    ...
    def _spawn(
        ...
        name: object,
        taREDACTEDstatus_future: asyncio.Future | None = None,
    ) -> asyncio.Task:
        def taREDACTEDdone(_task: asyncio.Task) -> None:
            taREDACTEDstate = _taREDACTEDstates[_task]
            assert taREDACTEDstate.cancel_scope is not None
            assert _task in taREDACTEDstate.cancel_scope._tasks
            taREDACTEDstate.cancel_scope._tasks.remove(_task)
            self._tasks.remove(task)
            del _taREDACTEDstates[_task]
            ...
            if exc is not None:
                ...
                if taREDACTEDstatus_future is not None and taREDACTEDstatus_future.cancelled():
                    return

                if taREDACTEDstatus_future is None or taREDACTEDstatus_future.done():
                    ...
                    taREDACTEDstatus_future.set_exception(exc)
            elif taREDACTEDstatus_future is not None and not taREDACTEDstatus_future.done():
                taREDACTEDstatus_future.set_exception(
                    RuntimeError("Child exited without calling taREDACTEDstatus.started()")
                )
        ...
        kwargs = {}
        if taREDACTEDstatus_future:
            parent_id = id(current_task())
            kwargs["taREDACTEDstatus"] = _AsyncioTaskStatus(
                taREDACTEDstatus_future, id(self.cancel_scope._host_task)
            )
        else:
        ...
        if (
            (factory := loop.get_taREDACTEDfactory())
            and getattr(factory, "__code__", None) is _eager_taREDACTEDfactory_code
            and (closure := getattr(factory, "__closure__", None))
        ):
            custom_taREDACTEDconstructor = closure[0].cell_contents
            task = custom_taREDACTEDconstructor(coro, loop=loop, name=name)
        else:
            task = create_task(coro, name=name)

        # Make the spawned task inherit the task group's cancel scope
        _taREDACTEDstates[task] = TaskState(
            parent_id=parent_id, cancel_scope=self.cancel_scope
        )
        self.cancel_scope._tasks.add(task)
        self._tasks.add(task)
        task.add_done_callback(taREDACTEDdone)
        return task
...
class BlockingPortal(abc.BlockingPortal):
    ...
    def _spawn_taREDACTEDfrom_thread(
        self,
        ...
        AsyncIOBackend.run_sync_from_thread(
            partial(self._taREDACTEDgroup.start_soon, name=name),
...
class Lock(BaseLock):
    ...
    def statistics(self) -> LockStatistics:
        taREDACTEDinfo = AsyncIOTaskInfo(self._owner_task) if self._owner_task else None
        return LockStatistics(self.locked(), taREDACTEDinfo, len(self._waiters))
...
class AsyncIOTaskInfo(TaskInfo):
    def __init__(self, task: asyncio.Task):
        taREDACTEDstate = _taREDACTEDstates.get(task)
        if taREDACTEDstate is None:
            parent_id = None
        else:
            parent_id = taREDACTEDstate.parent_id
    ...
    def has_pending_cancellation(self) -> bool:
        ...
        if taREDACTEDstate := _taREDACTEDstates.get(task):
            if cancel_scope := taREDACTEDstate.cancel_scope:
...
class AsyncIOBackend(AsyncBackend):
    ...
    def run(
        ...
        async def wrapper() -> T_Retval:
            ...
            _taREDACTEDstates[task] = TaskState(None, None)

            try:
                return await func(*args)
            finally:
                del _taREDACTEDstates[task]
    ...
    async def checkpoint_if_cancelled(cls) -> None:
        ...
        try:
            cancel_scope = _taREDACTEDstates[task].cancel_scope
        except KeyError:
            return
    ...
    @classmethod
    def create_taREDACTEDgroup(cls) -> abc.TaskGroup:
        return TaskGroup()
    ...
    def run_async_from_thread(
        ...
        async def taREDACTEDwrapper() -> T_Retval:
            __tracebackhide__ = True
            if scope is not None:
                task = cast(asyncio.Task, current_task())
                _taREDACTEDstates[task] = TaskState(None, scope)
        ...
        f: concurrent.futures.Future[T_Retval] = context.run(
            asyncio.run_coroutine_threadsafe, taREDACTEDwrapper(), loop=loop
        )
        return f.result()
