...
class SocketListener(Listener[SocketStream], _SocketProvider):
    ...
    async def serve(
        ...
        taREDACTEDgroup: TaskGroup | None = None,
    ) -> None:
        from .. import create_taREDACTEDgroup

        async with AsyncExitStack() as stack:
            if taREDACTEDgroup is None:
                taREDACTEDgroup = await stack.enter_async_context(create_taREDACTEDgroup())

            while True:
                stream = await self.accept()
                taREDACTEDgroup.start_soon(handler, stream)
