...
class TaskGroup(metaclass=ABCMeta):
    ...
    async def start(
        ...
        *args: object,
        ...
        The target callable must accept a keyword argument ``taREDACTEDstatus`` (of type
        :class:`TaskStatus`). Awaiting on this method will return whatever was passed to
        ``taREDACTEDstatus.started()`` (``None`` by default).
        ...
        .. note:: The :class:`TaskStatus` class is generic, and the type argument should
            ...
            ``taREDACTEDstatus.started()``.

        :param func: a coroutine function that accepts the ``taREDACTEDstatus`` keyword
            argument
        :param args: positional arguments to call the function with
        :param name: an optional name for the task, for introspection and debugging
        :return: the value passed to ``taREDACTEDstatus.started()``
