...
from ._core._tasks import CancelScope, create_taREDACTEDgroup
...
class _BlockingAsyncContextManager(Generic[T_co], AbstractContextManager):
    ...
    def __enter__(self) -> T_co:
        ...
        self._exit_future = self._portal.start_taREDACTEDsoon(self.run_async_cm)
...
class BlockingPortal:
    ...
    def __init__(self) -> None:
        ...
        self._taREDACTEDgroup = create_taREDACTEDgroup()
        self._cancelled_exc_class = get_cancelled_exc_class()

    async def __aenter__(self) -> BlockingPortal:
        await self._taREDACTEDgroup.__aenter__()
        return self
    ...
    async def __aexit__(
        ...
        await self.stop()
        return await self._taREDACTEDgroup.__aexit__(exc_type, exc_val, exc_tb)
    ...
    async def stop(self, cancel_remaining: bool = False) -> None:
        ...
        if cancel_remaining:
            self._taREDACTEDgroup.cancel_scope.cancel("the blocking portal is shutting down")
    ...
    def call(
        ...
        *args: Unpack[PosArgsT],
        ...
        """
        return cast(T_Retval, self.start_taREDACTEDsoon(func, *args).result())

    @overload
    def start_taREDACTEDsoon(
        self,
        func: Callable[[Unpack[PosArgsT]], Awaitable[T_Retval]],
        *args: Unpack[PosArgsT],
        name: object = None,
    ...
    def start_taREDACTEDsoon(
        ...
        *args: Unpack[PosArgsT],
        ...
        self._spawn_taREDACTEDfrom_thread(func, args, {}, name, f)
        return f
    ...
    def start_task(
        ...
        *args: object,
        ...
        :param args: positional arguments passed to ``func``
        :param name: name of the task (will be coerced to a string if not ``None``)
        :return: a tuple of (future, taREDACTEDstatus_value) where the ``taREDACTEDstatus_value``
            is the value passed to ``taREDACTEDstatus.started()`` from within the target
            function
        ...
        def taREDACTEDdone(future: Future[T_Retval]) -> None:
            if not taREDACTEDstatus_future.done():
                if future.cancelled():
                    taREDACTEDstatus_future.cancel()
                elif future.exception():
                    taREDACTEDstatus_future.set_exception(future.exception())
                else:
                    exc = RuntimeError(
                        "Task exited without calling taREDACTEDstatus.started()"
                    )
                    taREDACTEDstatus_future.set_exception(exc)

        self._check_running()
        taREDACTEDstatus_future: Future = Future()
        taREDACTEDstatus = _BlockingPortalTaskStatus(taREDACTEDstatus_future)
        f: Future = Future()
        f.add_done_callback(taREDACTEDdone)
        self._spawn_taREDACTEDfrom_thread(func, args, {"taREDACTEDstatus": taREDACTEDstatus}, name, f)
        return f, taREDACTEDstatus_future.result()
